...
function LogoModel() {
  ...
  useFrame((state, delta) => {
    ...
    groupRef.current.position.y = THREE.MathUtils.damp(groupRef.current.position.y, -0.25 + Math.sin(elapsed * 1.35) * 0.06, 3.8, delta);
  ...
  return (
    <group ref={groupRef}>
      <Float speed={1.1} rotationIntensity={0.1} floatIntensity={0.12}>
        <primitive ref={modelRef} object={preparedModel} scale={2.2} position={[0, -0.3, 0]} />
      </Float>
    </group>
  );
}
  ...
  return (
    <div className={styles.wrapper}>
      ...
      >
        ...
        <ContactShadows opacity={0.35} scale={6} blur={1.5} far={4.5} resolution={512} position={[0, -1.55, 0]} />
